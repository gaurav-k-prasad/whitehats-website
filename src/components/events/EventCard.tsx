  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (frame <= fullScrambleFrames) {
        // Phase 1: 100% of the entire text (including first line/words) scrambles
        setDisplayText(
          originalText
            .split("")
            .map((char) =>
...
export default function EventCard({ event }: EventCardProps) {
  ...
  return (
    <motion.div
      ...
      className="bg-[#0B1120] border border-[#1E293B] hover:border-cyber-blue rounded-xl overflow-hidden relative group transition-all duration-300 hover:shadow-[0_0_25px_rgba(0,136,255,0.2)] flex flex-col justify-between select-none"
    >
      <div>
